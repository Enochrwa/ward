...
const WardrobeManager = () => {
  ...
  const handleSaveItem = async (newItemData: WardrobeItemCreate) => {
    if (!token) {
      toast({ title: "Authentication Error", description: "You must be logged in to add items.", variant: "destructive" });
      return;
    }
    try {
      const savedItem = await apiClient('/wardrobe/items/', {
        method: 'POST',
        body: newItemData,
      });
  ...
  // Handle Update Item
  const handleUpdateItem = async (itemId: string, updatedData: Partial<WardrobeItemCreate | { favorite: boolean }>) => {
    if (!token) {
    ...
    // Optimistic update
    setItems(prevItems => prevItems.map(item => item.id === itemId ? { ...item, ...updatedData } : item));

    try {
      await apiClient(`/wardrobe/items/${itemId}`, {
        method: 'PUT', // Or PATCH if backend supports partial updates for all fields
        body: updatedData,
      });
      toast({ title: "Success", description: "Item updated successfully." });
      // Optionally re-fetch only if PUT doesn't return the full updated item or for strict consistency
      // For now, optimistic update is considered sufficient, or a targeted fetch if needed.
      // fetchItems();
    } catch (err: any) {
      ...
      // fetchItems(); // Optionally re-fetch to ensure UI consistency after error
    }
  };
